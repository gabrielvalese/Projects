...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['salvar'])) {
        if (!empty($emailUserPK)) {
            $usuarioAtual = listEmail($mysqli, $emailUserPK);

            if (!empty($usuarioAtual)) {
                $dadosAtuais = $usuarioAtual[0];
                $senhaParaSalvar = !empty($senhaEdit)
                    ? password_hash($senhaEdit, PASSWORD_BCRYPT)
                    : $dadosAtuais['senha'];

                $nomeParaSalvar = !empty($nomeEdit) ? $nomeEdit : $dadosAtuais['nome'];
                $cpfParaSalvar = !empty($cpfEdit) ? $cpfEdit : $dadosAtuais['cpf'];
                $nascimentoParaSalvar = !empty($nascimentoEdit) ? $nascimentoEdit : $dadosAtuais['nascimento'];
            ...
            $mensagem = "Nenhum usuário foi selecionado para edição.";
        }
    }
}
?>
...
<div class="card shadow-sm">
    <div class="card-body">
        ...
        <?php endif; ?>
            ...
            <button type="submit" class="btn btn-warning" name="salvar" value="1">Salvar Alterações</button>
